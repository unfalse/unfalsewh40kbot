...
export class AskCommandHandler {
  ...
  async handle(ctx: Context): Promise<void> {
    ...
    if (!query) {
      await ctx.reply(
        t(messages.handlers.ask.empty_query, lang),
    ...
    try {
      ...
      const result = await this.llm.wrapInPersona(query, "plain", lang, history, sysEnabled);
      this.conversation.push(userId, "user", query);
      ...
      await ctx.reply(
        t(messages.handlers.ask.error, lang),
        messageId ? { reply_parameters: { message_id: messageId }, parse_mode: "HTML" } : { parse_mode: "HTML" },
      );
    }
  }
