...
export class ReplyHandler {
  ...
  async handle(ctx: Context): Promise<void> {
    ...
    // Обработываем только если это ответ на сообщение и есть текст
    if (!replyTo || !text || text.trimStart().startsWith("/")) return;

    // Пропускаем если это ответ на сообщение от самого пользователя (не от бота)
    ...
    try {
      ...
      console.error("[ReplyHandler] error:", reason);
      await ctx.reply(
